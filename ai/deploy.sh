...
command="az webapp config appsettings set -g $resource_group --name $name --settings USER_AGENT=promptflow-appservice "
command=$(append_to_command "$command")
echo "$command"
eval "$command"
echo "You need to add connections to the app as env variables manually!"
echo "Please go to https://portal.azure.com/ to config environment variables and restart the app: $name at (Settings>Configuration)"
